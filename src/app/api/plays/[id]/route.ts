...
import { getPlayBySlug } from "@/lib/plays";
...
export async function PATCH(req: Request, { params }: Ctx) {
  ...
  try {
    await connectDB();
    const doc = await Play.findById(id);
    ...
    if (body.repoUrl !== undefined) doc.repoUrl = body.repoUrl?.trim() || undefined;
    ...
    if (body.slug !== undefined) {
      ...
      let slug = base;
      for (let i = 2; await Play.exists({ slug, _id: { $ne: doc._id } }); i++) slug = `${base}-${i}`;
      doc.slug = slug;
    }

    await doc.save();
    return Response.json(await getPlayBySlug(doc.slug));
  } catch (err) {
    return serverError("PATCH /api/plays/[id] failed", err);
  }
}
