...
export default function PlaysPage() {
  ...
  const loadPage = useCallback(async (skip: number) => {
    ...
    try {
      const res = await fetch(`/api/plays?skip=${skip}&limit=${PAGE_SIZE}`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data: { items: PlayItem[]; hasMore: boolean } = await res.json();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/plays?skip=0&limit=${PAGE_SIZE}`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data: { items: PlayItem[]; hasMore: boolean } = await res.json();
        if (!active) return;
        setItems(data.items);
      ...
      <section className="mx-auto w-full max-w-7xl px-6 pb-28">
        <MasonryGrid
          ...
          renderCard={(item, index) => (
            <NewsCard item={item} basePath="plays" priority={index === 0} openExternal={false} />
          )}
          onLoadMore={loadMore}
          hasMore={hasMore}
        />
