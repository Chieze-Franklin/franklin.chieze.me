...
import type { PlayItem, CardSize, WorkLink } from "@/types";
...
function toForm(w: PlayItem): FormState {
  return {
    _id: w._id,
    title: w.title,
    slug: w.slug,
...
export function AdminPlays() {
  ...
  const refresh = useCallback(async () => {
    const res = await fetch("/api/plays?limit=100");
    const data = await res.json();
    if (!res.ok) {
      setListError(data.error || `Failed to load plays (HTTP ${res.status})`);
      setLoading(false);
      return;
    }
    setListError("");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const data = await res.json();
      if (!active) return;
      if (!res.ok) setListError(data.error || `Failed to load plays (HTTP ${res.status})`);
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = form._id
        ? await fetch(`/api/plays/${form._id}`, {
            method: "PATCH",
  ...
  const remove = async (item: PlayItem) => {
    if (!confirm(`Delete "${item.title}"? This also removes its images.`)) return;
    const res = await fetch(`/api/plays/${item._id}`, { method: "DELETE" });
      ...
      <div className="grid gap-3">
        ...
        {!loading && items.length === 0 && (
          <p className="py-10 text-center text-sm" style={{ color: "var(--text-secondary)" }}>
            No play items yet. Add your first one.
          </p>
        )}
      </div>
          ...
          >
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-xl font-black" style={{ color: "var(--text-primary)" }}>
                {form._id ? "Edit play item" : "New play item"}
              </h2>
            ...
            <div className="grid gap-4">
              ...
              <Field label="Slug (optional — generated from title if blank)">
                <input className="admin-input" value={form.slug} onChange={(e) => update({ slug: e.target.value })} placeholder="my-experiment" />
              </Field>
              <Field label="Summary">
              ...
              <Field label="Description (optional)">
                <textarea className="admin-input" rows={5} value={form.content} onChange={(e) => update({ content: e.target.value })} placeholder="Full write-up shown on the play page" />
              </Field>
              ...
              <Field label="Tags (comma-separated)">
                <input className="admin-input" value={form.tags} onChange={(e) => update({ tags: e.target.value })} placeholder="threejs, 3d, portfolio" />
              </Field>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="Repository URL">
                  <input type="url" className="admin-input" value={form.repoUrl} onChange={(e) => update({ repoUrl: e.target.value })} placeholder="https://github.com/…" />
                </Field>
              </div>
